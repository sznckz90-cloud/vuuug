import {
  ...
  type DailyAdTasksConfig,
  type InsertDailyAdTasksConfig,
  type UserDailyAdTask,
  type InsertUserDailyAdTask,
} from "../shared/schema";
...
export class DatabaseStorage implements IStorage {
  ...
  // ===== DAILY AD TASKS SYSTEM =====
  
  // Get current reset date in YYYY-MM-DD format (resets at 00:00 UTC)
  private getCurrentDailyAdResetDate(): string {
    const now = new Date();
    return now.toISOString().split('T')[0];
  }

  // Get all daily ad task configurations (for admin)
  async getDailyAdTasksConfig(): Promise<DailyAdTasksConfig[]> {
    return db
      .select()
      .from(dailyAdTasksConfig)
      .orderBy(dailyAdTasksConfig.displayOrder);
  }

  // Update daily ad task configuration (admin)
  async updateDailyAdTaskConfig(
    taskLevel: number, 
    updates: { requiredAds?: number; rewardType?: string; rewardAmount?: string; displayOrder?: number; isEnabled?: boolean }
  ): Promise<DailyAdTasksConfig | null> {
    const [updated] = await db
      .update(dailyAdTasksConfig)
      .set({
        ...updates,
        updatedAt: new Date()
      })
      .where(eq(dailyAdTasksConfig.taskLevel, taskLevel))
      .returning();
    return updated || null;
  }

  // Create new daily ad task (admin)
  async createDailyAdTask(task: InsertDailyAdTasksConfig): Promise<DailyAdTasksConfig> {
    const [created] = await db
      .insert(dailyAdTasksConfig)
      .values(task)
      .returning();
    return created;
  }

  // Delete daily ad task (admin)
  async deleteDailyAdTask(taskLevel: number): Promise<boolean> {
    const result = await db
      .delete(dailyAdTasksConfig)
      .where(eq(dailyAdTasksConfig.taskLevel, taskLevel));
    return true;
  }

  // Get user's daily ad tasks for today (creates if not exist)
  async getUserDailyAdTasks(userId: string): Promise<(UserDailyAdTask & { config: DailyAdTasksConfig })[]> {
    const resetDate = this.getCurrentDailyAdResetDate();
    
    // Get all enabled task configs
    const configs = await db
      .select()
      .from(dailyAdTasksConfig)
      .where(eq(dailyAdTasksConfig.isEnabled, true))
      .orderBy(dailyAdTasksConfig.displayOrder);
    ...
    // Get existing user tasks for today
    let existingTasks = await db
      .select()
      .from(userDailyAdTasks)
      .where(and(
        eq(userDailyAdTasks.userId, userId),
        eq(userDailyAdTasks.resetDate, resetDate)
      ))
      .orderBy(userDailyAdTasks.taskLevel);

    // If no tasks exist for today, create them
    if (existingTasks.length === 0) {
      const tasksToInsert: InsertUserDailyAdTask[] = configs.map(config => ({
        userId,
      ...
      await db.insert(userDailyAdTasks).values(tasksToInsert);
    ...
    // Merge tasks with configs
    return existingTasks.map(task => {
      const config = configs.find(c => c.taskLevel === task.taskLevel);
      return {
        ...task,
        config: config!
      };
    }).filter(t => t.config); // Only return tasks with valid configs
  }

  // Update daily ad task progress when user watches ads
  async updateDailyAdTaskProgress(userId: string): Promise<void> {
    const resetDate = this.getCurrentDailyAdResetDate();
    
    // Get user's current daily ad count
    const [user] = await db.select().from(users).where(eq(users.id, userId));
    if (!user) return;
    ...
    const tasks = await this.getUserDailyAdTasks(userId);
    
    // Find current active task (first unclaimed task where all previous are claimed)
    let currentTask: (UserDailyAdTask & { config: DailyAdTasksConfig }) | null = null;
    
    for (const task of tasks) {
      if (!task.claimed) {
        // Check if all previous tasks are claimed
        const previousTasks = tasks.filter(t => t.config.displayOrder < task.config.displayOrder);
    ...
    if (!currentTask) return; // All tasks claimed or no valid task

    // Calculate progress based on cumulative ads needed
    // For progressive system: Task 1 needs 100 ads, Task 2 needs 200 (cumulative from start), etc.
    const previousTasksAdsRequired = tasks
      .filter(t => t.config.displayOrder < currentTask!.config.displayOrder && t.claimed)
      .reduce((sum, t) => sum + t.config.requiredAds, 0);
    
    // Progress for current task = total ads watched - ads consumed by previous tasks
    const adsAvailableForCurrentTask = Math.max(0, adsWatchedToday - previousTasksAdsRequired);
    const newProgress = Math.min(adsAvailableForCurrentTask, currentTask.config.requiredAds);
    const isCompleted = newProgress >= currentTask.config.requiredAds;

    // Update current task progress
    await db
      .update(userDailyAdTasks)
      .set({
        progress: newProgress,
        completed: isCompleted,
        completedAt: isCompleted && !currentTask.completed ? new Date() : currentTask.completedAt,
      ...
      .where(and(
        eq(userDailyAdTasks.userId, userId),
        eq(userDailyAdTasks.taskLevel, currentTask.taskLevel),
        eq(userDailyAdTasks.resetDate, resetDate)
      ));

    // Reset progress of future tasks (they start fresh when activated)
    await db
      .update(userDailyAdTasks)
      .set({
        progress: 0,
      ...
      .where(and(
        eq(userDailyAdTasks.userId, userId),
        eq(userDailyAdTasks.resetDate, resetDate),
        sql`${userDailyAdTasks.taskLevel} > ${currentTask.taskLevel}`,
        eq(userDailyAdTasks.claimed, false)
      ));
  }

  // Claim daily ad task reward
  async claimDailyAdTaskReward(userId: string, taskLevel: number): Promise<{ success: boolean; message: string; rewardAmount?: string; rewardType?: string }> {
    const resetDate = this.getCurrentDailyAdResetDate();
    
    // Get the task with its config
    const tasks = await this.getUserDailyAdTasks(userId);
    const task = tasks.find(t => t.taskLevel === taskLevel);
    ...
    // Check sequential requirement - all previous tasks must be claimed
    const previousTasks = tasks.filter(t => t.config.displayOrder < task.config.displayOrder);
    const allPreviousClaimed = previousTasks.every(t => t.claimed);
    ...
    await db
      ...
      .where(and(
        eq(userDailyAdTasks.userId, userId),
        eq(userDailyAdTasks.taskLevel, taskLevel),
        eq(userDailyAdTasks.resetDate, resetDate)
      ));

    const rewardAmount = task.config.rewardAmount;
    const rewardType = task.config.rewardType;

    // Add reward based on type
    if (rewardType === 'wiew') {
      // Add to PAD/Wiew balance (balance column)
      await this.addEarning({
        userId,
        amount: rewardAmount,
        source: 'daily_ad_task',
        description: `Daily Ad Task ${taskLevel} reward: ${rewardAmount} Wiew`,
      });
    } else if (rewardType === 'usd') {
      // Add directly to USD balance
      await db
        .update(users)
        .set({
          usdBalance: sql`COALESCE(${users.usdBalance}, 0) + ${rewardAmount}`,
          updatedAt: new Date(),
        })
        .where(eq(users.id, userId));
      ...
      await this.logTransaction({
        ...
        amount: rewardAmount,
        type: 'addition',
        source: 'daily_ad_task_usd',
        description: `Daily Ad Task ${taskLevel} USD reward`,
        metadata: { taskLevel, rewardType: 'usd', resetDate }
      });
    }
  ...
  // Admin: Reset a specific user's daily ad task progress
  async resetUserDailyAdTasks(userId: string): Promise<boolean> {
    const resetDate = this.getCurrentDailyAdResetDate();
    
    await db
      .delete(userDailyAdTasks)
      .where(and(
        eq(userDailyAdTasks.userId, userId),
        eq(userDailyAdTasks.resetDate, resetDate)
      ));
  ...
  // Get daily ad task summary for a user
  async getDailyAdTaskSummary(userId: string): Promise<{
    tasks: (UserDailyAdTask & { config: DailyAdTasksConfig })[];

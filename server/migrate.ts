...
export async function ensureDatabaseSchema(): Promise<void> {
  try {
    ...
    try {
      ...
      // Add hourly ad limit tracking columns (December 2025)
      await db.execute(sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS ads_watched_this_hour INTEGER DEFAULT 0`);
      await db.execute(sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS hourly_window_start TIMESTAMP`);
